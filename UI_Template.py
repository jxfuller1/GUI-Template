...
from PyQt5.QtWidgets import QApplication, QMainWindow, QHBoxLayout, QWidget,QVBoxLayout, QStackedLayout, \
    QStackedWidget, QScrollArea, QSizePolicy, QSplitter, QToolBar, QAction, QToolButton, QStatusBar

# UI portions of each specific program offloaded to separate file
import UI_Template_Program1
import UI_Template_program_generic
...
total_programs = ["Program_1", "Generic"]
# enter number of buttons for each program and their names that you want to appear on left side list in UI
program_buttons = [["program1 Main", "program1 Setup", "program1 Misc", "program1 About", "program1 Extra"],
                   ["soemthing1", "something2"]]
...
class Actions(QMainWindow):
    ...
    def __init__(self):
        ...
        self.program1_mainwork_layout = UI_Template_Program1.Program1_Layout(self)
        self.program_generic = UI_Template_program_generic.Program_generic_layout(self)

        # put all programs into a list for the mainwork_ui_layout function
        self.program_list = [self.program1_mainwork_layout, self.program_generic]
    ...
    def initUI(self):
        ...
        self.setWindowTitle("UI_Template")
        ...
        for i in self.program_list:
            ...
            # signal connection for program option that changes size of window
            i.sizeChanged.connect(self.onsizeChanged)
        ...
        # use status bar for program updates  (not currently being used atm)
    ...
    # activates when program changes settings sizes for window, changes window size
    def onsizeChanged(self, width, height):
        ...
        # resize window
        self.mainwindow_animation.setEndValue(QSize(width, height))
    ...
    def switch_leftsidebuttons_UI(self, sender):
        ...
        # # activate function from program1 to auto resize window to sizes listing in the settings in program1
        if self.top_button_layout.indexOf(sender) == 0:
            # activate function from program1 to auto resize window to sizes listing in the settings in program1
            self.program1_mainwork_layout.start_program1()
